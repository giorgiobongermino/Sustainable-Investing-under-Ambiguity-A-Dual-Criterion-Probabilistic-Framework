...
from dpu_worker_CVaR import run_one_dpu_job  # worker con FIN mean–CVaR empirico & ESG mean–variance

# ===== CVaR empirico: livello di confidenza usato dal worker =====
ALPHA_FIN = 0.95
...
plt.hist(mean_ret)
plt.title("Distribuzione dei ritorni medi")
plt.xlabel("Ritorno medio")
plt.ylabel("Frequenza")
plt.show()
plt.hist(mean_ret/np.sqrt(np.diag(Sigma_fin_np)),bins=20)
plt.title("Distribuzione degli sharpe")
plt.xlabel("Sharpe")
plt.ylabel("Frequenza")
plt.show()
plt.hist(np.sqrt(np.diag(Sigma_fin_np)),bins=20)
plt.title("Distribuzione delle varianze")
plt.xlabel("Varianza")
plt.ylabel("Frequenza")
plt.show()

# ====== scale k (come nel tuo codice) e ν coerenti (SNR-based) ======
k = float(np.mean(mu_esg_np / np.sqrt(np.clip(np.diag(Sigma_esg_np), 1e-12, np.inf))))
...
print(f"[INFO] n_assets={n_assets} | k={k:.2f} | snr_fin={snr_fin:.2g} | snr_esg={snr_esg:.2g}|nu_fin={nu_fin:.2g} | snr_esg={nu_esg:.2g}")
...
# ========== 3) COSTRUZIONE JOBS ==========
# Nota: ora passiamo anche fin_df (returns %) e alpha_fin al worker (mean–CVaR empirico)
...
for key, m, lfin, lesg in combos:
    jobs.append({
        ...
        "fin_np":fin_np,"fin_mask":fin_mask,
        # ESG mean–variance
        ...
        # riproducibilità (se expectation_DPU lo supporta)
        "rng_seed": int(rng.integers(0, 2**31 - 1)),

        # ====== nuovi campi per mean–CVaR empirico ======
        "alpha_fin": float(ALPHA_FIN),    # livello di confidenza CVaR
        "fin_df": returns,                # storico rendimenti % in-sample per CVaR empirico
        # (non serve esg_df qui, il worker usa μ/Σ per ESG)
    })
...
if __name__ == "__main__":
    ...
    df.to_csv("parallel_meu_dpu_results_CVaR.csv", index=False)
    ...
    np.savez_compressed("weights_meu_CVaR.npz", **weights_meu)
    np.savez_compressed("weights_dpu_CVaR.npz", **weights_dpu)

    # opzionale: anche pickle
    import pickle
    with open("weights_meu_CVaR.pkl", "wb") as f:
        pickle.dump(weights_meu, f, protocol=pickle.HIGHEST_PROTOCOL)
    with open("weights_dpu_CVaR.pkl", "wb") as f:
